...
Technical choices: files are pre-optimized WebP in `public/images/` (hero about 220 KB, others 20 to 130 KB, EXIF stripped) and served with `unoptimized` next/image, so no Vercel image-optimization usage is billed. Text over photos sits on CSS overlays; decorative backgrounds have empty alt text. All 15 blog posts have their own photo (Pexels, same rules) as a hero image and a smaller card thumbnail; alt text is written per image and a test enforces the size budget (150 KB) and that every image exists. Social cards still use no photographs. Free stock sites do not verify model or property releases; paid stock is the option if indemnification matters.

### Decision 58 -- Signup now says plainly when an email already has an account

Owner decision, September 19, 2026, after signing up with an address that already had an account and being left waiting for an email that never came (Supabase creates nothing and sends nothing in that case, and reports success). Signing up with an email that belongs to a **confirmed** account now returns "An account with this email already exists." with links to log in or reset the password, and creates nothing.
...
### Decision 61 -- The site is named bulkwinegrapes.com, "BWG" for short

Owner decision, September 19, 2026. "HarvestLink" is retired as the public name. The header, footer, sign-in screens and dashboard now show the supplied bulkwinegrapes.com wordmark (`public/brand/bwg-wordmark.png`, background made transparent; source image trimmed and un-matted from the file the owner sent).

- **Naming rule:** the full domain where the name defines or identifies the site (page title of the homepage, footer and legal definitions, "About" boxes, JSON-LD name, email sender and email text, the "What is bulkwinegrapes.com?" FAQ); **BWG** everywhere else (page-title suffixes, body copy, guides, region and variety pages, dashboard labels). The canonical description introduces both: "bulkwinegrapes.com (BWG) is an online marketplace ...". The Terms and Privacy Policy define the Site as "bulkwinegrapes.com" or "BWG".
- **Structured data:** Organization and WebSite are named `bulkwinegrapes.com` with alternate names `BWG` and `Bulk Wine Grapes`. `SITE_NAME` and `SITE_SHORT_NAME` in `src/lib/seo.ts` are the single source.
- **Social cards** now use the wordmark instead of the old grape mark and text (embedded in `src/lib/og-wordmark.ts`).
- **Brand files** renamed `harvestlink-*` to `bwg-*` (`bwg-mark.svg`, `bwg-logo-512.png`); the old wordmark files were deleted. The grape-cluster **favicon and app icon are unchanged** (the new logo has no symbol); tell me if you want them replaced.
- **Not changed, on purpose:** the npm package name, the Vercel project name and its `*.vercel.app` URL, applied SQL migrations (comments only), and the historical entries in this file, the changelog and `docs/seo` (they record what was true when written).
- **Owner steps:** (1) re-paste `supabase/email-templates/confirm-signup.html` and `reset-password.html` into Supabase (subjects now read "Your BWG code" and "Your BWG password reset code"); (2) if Supabase Auth has a **sender name** set to HarvestLink (Authentication, SMTP settings), change it; (3) the production `RESEND_FROM_EMAIL` is now `bulkwinegrapes.com <notifications@bulkwinegrapes.com>`.

### Decision 62 -- Wanted requests ("looking for" posts)

Owner request, September 19, 2026, built with the recommended defaults. Buyers (or growers) post the grapes or bulk wine they want; sellers respond. Off until migration `0011` is run and `NEXT_PUBLIC_FEATURE_WANTED=true` is set.

- **Defaults chosen:** any logged-in member may post (growers look for grapes too); **anonymous by default** (sellers see "Anonymous buyer"); the target price is **hidden unless the poster ticks "show"**; a responder may **optionally offer one of their own lots**; up to **10 open requests** per member; each request lives **60 days**, renewable, never past 120 days from posting.
- **Data (`wanted_requests`, `wanted_responses`, `wanted_messages`, `wanted_notifications`, `wanted_alert_optouts`):** the public API roles have **no access at all** (no policies, no grants). Every read and write goes through the server with the service role. A poster's user id is never exposed because `profiles_public` would turn it back into a company name. `serializeWanted()` is an allowlist, so a new column stays private until it is added on purpose. Proven in a test database and by unit tests (`src/lib/wanted/wanted.test.ts`).
- **Responses** are separate, small threads (`/responses/{id}`), not the listing-inquiry tables, so the confidentiality logic in inquiries is untouched. Both sides can stay anonymous: the poster is "Anonymous buyer", and a responder who ticks "confidential", or who offers a lot listed under NDA, is "Confidential Seller". An attached lot is titled for the person looking, so a confidential lot's hidden detail stays hidden from the buyer. Emails follow the same naming rule.
- **Note text (500 characters)** is screened: no email, phone, link or profanity, and on an anonymous request no mention of the poster's own name, company or username. Titles and slugs are built only from controlled fields, never from the note.
- **Matching:** (1) when a buyer posts, "Email me when a matching lot is listed" (on by default) creates ordinary saved searches, one per region up to three, so the existing daily digest does the work; (2) the daily cron emails sellers whose available lots match a new request (same type and variety, region in the list if given, harvest year for grapes), once per request, one email per seller per run, capped at 100 sellers per request, never the poster. Those emails carry a signed one-click **unsubscribe** (`/wanted/unsubscribe`, HMAC of the user id with `CRON_SECRET`, plus the List-Unsubscribe header). They describe the request only, never who posted it or a private price.
- **Where it shows up:** a third segment "Wanted" in the header switch; a "Looking for something specific?" band on the homepage; a "Post a request" prompt on empty Grapes and Bulk Wine results, prefilled from the filters; "Buyers are looking for X" on every region and variety page; "Post it as a request" after saving a search; My Requests in the dashboard; the sitemap (open requests only), `llms.txt`, FAQ (3 answers), Terms section 5 and the Privacy Policy.
- **Header:** to keep it short, Blog, FAQ, Regions and Varieties are now inside a single **Explore** menu, and Wanted joins the existing Grapes / Bulk Wine switch instead of adding another link.
- **Not built (easy follow-ups):** editing a posted request (close and repost instead), an admin moderation queue for requests (an admin can delete rows in Supabase for now), and matching a bulk wine request's vintage in the seller emails.
- **Owner steps:** run `supabase/migrations/0011_wanted_requests.sql` in the Supabase SQL Editor, then set `NEXT_PUBLIC_FEATURE_WANTED=true` in Vercel and redeploy. Rollback: `supabase/rollback/0011_rollback.sql`.
