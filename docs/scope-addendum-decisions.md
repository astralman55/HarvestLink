...
### Decision 46 -- OPEN: existing identifying columns are still readable through the public API on NDA listings
...
Not fixed here because the real fix is a dedicated change: revoke column-level `SELECT` on those columns from `anon`/`authenticated`, and move every listing read (browse, detail, owner dashboard, edit, admin, vineyard typeahead) onto the server-side service-role client so the serializer becomes the only door. That touches most of the app's read paths and should ship with its own testing. **Recommendation: do this before actively promoting NDA listings**, since the homepage now advertises them.
